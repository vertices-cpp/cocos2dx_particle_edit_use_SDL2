...
void ParticleExample::setStyle(PatticleStyle style)
{
    ...
    if (_texture == nullptr)
    {
       ...
       if (!texture) return;
        ...
        if (!texture->loadMemData(_render,(unsigned char *)__firePngData, sizeof(__firePngData)))
           {
               delete texture;
                return  ;
           }
        ...
        setTexture(texture);
           Size size = texture->getImageSize();
        //  _position = (Vec2)((size - _contentSize) / 2);
          _center = size / 2;
    }
